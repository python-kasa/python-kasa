...
@device_smart
async def test_wifi_scan_login_version_3(dev: SmartDevice, mocker: MockerFixture):
    """Test wifi_scan for login_version 3 uses scanApList onboarding scan query."""
    mocker.patch.object(dev.config.connection_type, "login_version", 3)
    dev._discovery_info = {"region": "US"}
    ...
    query_spy = mocker.patch.object(dev.protocol, "query", return_value=mock_response)

    networks = await dev.wifi_scan()

    query_spy.assert_called_once_with({"scanApList": {"onboarding": {"scan": {}}}})
    assert len(networks) == 1
    net = networks[0]
    assert net.ssid == "test_ssid"
    assert net.bssid == "00:11:22:33:44:55"
    assert net.cipher_type == "WPA2"
    assert net.key_type == "wpa2_psk"
    assert net.channel == 6
    assert net.signal_level == -40
