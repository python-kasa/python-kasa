...
async def test_smartcam_set_time_formats_local_time_as_utc_unit(
    monkeypatch: pytest.MonkeyPatch,
    mocker: MockerFixture,
):
    """Verify SmartCam set_time sends local_time in UTC without microseconds.

    Regression test for:
    - local_time using the machine's local timezone instead of UTC
    - local_time including microseconds (device expects 'YYYY-MM-DD HH:MM:SS')
    - timing_mode not being set to 'manual' (device ignores clock updates in ntp mode)
    """
    from kasa.smartcam.modules.time import Time as CamTimeModule

    inst = object.__new__(CamTimeModule)
    # Simulate a camera device that reports timing_mode (TC40, C210, etc.)
    fake_data = {
        "getTimezone": {"system": {"basic": {"timing_mode": "ntp", "zone_id": "UTC"}}}
    }
    monkeypatch.setattr(CamTimeModule, "data", property(lambda self: fake_data))
    call_mock = mocker.patch.object(inst, "call", new=AsyncMock(return_value={}))

    # A known UTC timestamp: 2024-03-15 10:30:00 UTC
    test_dt = datetime(2024, 3, 15, 10, 30, 0, tzinfo=UTC)

    await CamTimeModule.set_time(inst, test_dt)

    assert call_mock.await_count == 2

    # First call: set timing_mode to manual (so device accepts the update)
    ...
    # Second call: set clock_status
    second_args, _ = call_mock.await_args_list[1]
    assert second_args[0] == "setTimezone"
    clock = second_args[1]["system"]["clock_status"]
    # local_time must be UTC, exactly 'YYYY-MM-DD HH:MM:SS' – no microseconds
    assert clock["local_time"] == "2024-03-15 10:30:00"
    assert clock["seconds_from_1970"] == int(test_dt.timestamp())


async def test_smartcam_set_time_with_zoneinfo_unit(
    monkeypatch: pytest.MonkeyPatch,
    mocker: MockerFixture,
):
    """Verify zone_id is combined with timing_mode in one basic call when ZoneInfo given."""
    from kasa.smartcam.modules.time import Time as CamTimeModule
    ...
    # 2024-03-15 12:30:00 Europe/Berlin = 2024-03-15 11:30:00 UTC
    test_dt = datetime(2024, 3, 15, 12, 30, 0, tzinfo=tz)

    await CamTimeModule.set_time(inst, test_dt)

    assert call_mock.await_count == 2

    # First call merges timing_mode + zone_id into one basic params call
    first_args, _ = call_mock.await_args_list[0]
    basic = first_args[1]["system"]["basic"]
    assert basic["timing_mode"] == "manual"
    assert basic["zone_id"] == "Europe/Berlin"

    # Second call: local_time must be UTC (11:30:00), not local Berlin time (12:30:00)
    second_args, _ = call_mock.await_args_list[1]
    clock = second_args[1]["system"]["clock_status"]
    assert clock["local_time"] == "2024-03-15 11:30:00"
    assert clock["seconds_from_1970"] == int(test_dt.timestamp())


async def test_smartcam_set_time_no_timing_mode_skips_basic_unit(
    monkeypatch: pytest.MonkeyPatch,
    mocker: MockerFixture,
):
    """Devices without timing_mode (e.g. H200) skip the basic setTimezone call."""
    from kasa.smartcam.modules.time import Time as CamTimeModule

    inst = object.__new__(CamTimeModule)
    # H200 hub fixture: no timing_mode field in basic
    fake_data = {
        "getTimezone": {
            "system": {"basic": {"timezone": "UTC+00:00", "zone_id": "Europe/London"}}
        }
    }
    monkeypatch.setattr(CamTimeModule, "data", property(lambda self: fake_data))
    call_mock = mocker.patch.object(inst, "call", new=AsyncMock(return_value={}))

    # UTC-aware datetime, non-ZoneInfo tzinfo → no zone_id update either
    test_dt = datetime(2024, 3, 15, 10, 30, 0, tzinfo=UTC)

    await CamTimeModule.set_time(inst, test_dt)

    # Only the clock_status call should be made; no basic call
    assert call_mock.await_count == 1
    args, _ = call_mock.await_args_list[0]
    assert args[0] == "setTimezone"
    assert "clock_status" in args[1]["system"]
